...
import ReportarFugaFlow from "./ReportarFugaFlow";
...
export default function ReportarForm() {
  ...
  async function enviar(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok || !data.ok) setError(data.error ?? "No se pudo enviar el reporte.");
      else setExito(true);
    } catch {
    ...
    return (
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 text-center">
        ...
        <div className="mt-5 flex flex-col sm:flex-row gap-3 justify-center">
          <a href="/" className="inline-flex items-center justify-center gap-2 bg-[color:var(--color-primary)] text-white font-medium px-5 py-2.5 rounded-xl hover:opacity-90 transition">
            Ver el mapa
          </a>
          <button onClick={reiniciar} className="inline-flex items-center justify-center gap-2 border border-slate-300 text-slate-700 font-medium px-5 py-2.5 rounded-xl hover:bg-slate-50 transition">
            Hacer otro reporte
          </button>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {tipo === null && (
        <p className="text-sm text-slate-500">Elige una opción de arriba para empezar.</p>
      )}

      {/* Fuga: flujo rápido con foto + ubicación + IA */}
      {tipo === "fuga" && <ReportarFugaFlow />}

      {/* Tandeo / mala calidad: formulario normal */}
      {(tipo === "tandeo" || tipo === "mala_calidad") && (
        <form onSubmit={enviar} className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="colonia" className="block text-sm font-semibold text-slate-700 mb-1.5">Colonia</label>
              <input
                id="colonia"
                value={colonia}
            ...
            <div>
              <label htmlFor="municipio" className="block text-sm font-semibold text-slate-700 mb-1.5">Municipio o alcaldía</label>
              <input
                id="municipio"
          ...
          <div>
            <label htmlFor="estado" className="block text-sm font-semibold text-slate-700 mb-1.5">Estado</label>
            <select
              id="estado"
              value={estado}
          ...
          <p className="flex items-start gap-2 text-xs text-slate-500">
            ...
            Tu reporte es anónimo. Guardamos solo tu colonia (nunca tu dirección exacta).
          </p>
        </form>
      )}
    </div>
